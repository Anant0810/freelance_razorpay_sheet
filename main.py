...
import gspread
from utils.constants import CONFIG_CONSOLE, LOG_FILE, SHEET_NAME, START_DATE


warnings.simplefilter(action='ignore', category=FutureWarning)

directory = os.path.dirname(os.path.realpath(__file__))
cfg = gspread_pandas.conf.get_config(conf_dir=directory, file_name=CONFIG_CONSOLE)
...
cols = ['Start Date', 'datetime', 'payment page id', 'payment page name', 'id', 'status', 'created_at', 
            'vpa', 'name', 'email_notes', 'phone', 'amount', 'contact', 'email', 'webinar', 'notes']
...
#     print(data)
    ...
    data['webinar'] = data.apply(lambda x : get_flatter_df(x, k='webinar_name'), axis=1)
    ...
    captured_data_date = captured_data_date.copy()
    captured_data_date['amount'] = captured_data_date['amount'].astype(float) / 100
...
#     for payment_page_name, payment_page_id in payment_link_ids.items():
    ...
    while fetching:
        ...
        try:
            ...
            # print('error', e)
            # print(res.json())
            logger.infos(f"Something went wrong in payment fetch : {e}")
            sys.exit()
...
def get_razorpay_data(client, start_date):
    ...
    if start_date == None:
        ...
        # print(since)
        # try:
        print("payment link extract start")
...
#         since_d = since_dt + timedelta(days=1)
        ...
        try:
            ...
            message = f"Something went wrong in getting payment link data! {e}"
            dff = pd.DataFrame()
            logger.info(f"{client.capitalize()} - {message}")
...
def update_sheets(sheet, client):
    ...
    if has_sheet:
        try:
            since_dt = get_start_date(sheet, sheet_name, col='datetime') #  read the sheet and find the last date
        except Exception as e:
            print(e)
            logger.error(f"Error while fetching start date for {client.capitalize()} - {e}")
    ...
    try:
        ...
        logger.error(f"Error while updating sheet for {client.capitalize()} - {e}")
...
if __name__ == '__main__':
    ...
    for client in clients:
        ...
        try:
            ...
            logger.info(f"Error updating sheets for client {client}: {e}")
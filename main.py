...
file_handler = logging.FileHandler(os.path.join(log_directory, 'automate_razorpay.log'))
...
def get_time_range():
    today = (datetime.now(tz=tz) - timedelta(days=0)).date().strftime("%Y-%m-%d")
    yesterday = (datetime.now(tz=tz) - timedelta(days=1)).date().strftime("%Y-%m-%d")
...
def create_pl_df(payments, payment_page_id, payment_page_name, start_date, end_date, cols=cols):
    ...
    captured_data.loc[:, 'datetime'] = captured_data['created_at'].apply(lambda x :datetime.fromtimestamp(x, tz=tz))
    captured_data.loc[:, 'start_date'] = captured_data['datetime'].dt.strftime('%Y-%m-%d')
    captured_data_date =  captured_data[(captured_data['start_date'] >= start_date) & (captured_data['start_date'] < end_date)].copy()
    ...
    captured_data_date.loc[:, 'start_date'] = captured_data_date['datetime'].dt.strftime('%Y-%m-%d')
    ...
    payment_link_df.drop_duplicates(inplace=True)
...
def get_payment_link_data(client, start_date, end_date):
    ...
    for payment_page_name, payment_page_id in payment_link_ids.items():
        ...
        while fetching:
            ...
            skip= inc*100
            to = start_date
            fetching = datetime.fromtimestamp(res.json()['items'][-1]['created_at'], tz=tz).strftime('%Y-%m-%d') >= to
            print(payment_page_name, datetime.fromtimestamp(res.json()['items'][-1]['created_at'], tz=tz).strftime('%Y-%m-%d %H:%M'))
...
def update_sheets(client):
    ...
    new_df = pd.DataFrame()
#     # spread = Spread('TEST')
    spread = Spread('daily data razorpay clients')
    if spread.find_sheet(sheet_name):
        ...
        has_df = True
        try:
            since_dt = datetime.strptime(df['start_date'][0], '%Y-%m-%d').date()
        ...
        if has_df:
            ...
            since_dt = since_dt.strftime('%Y-%m-%d')

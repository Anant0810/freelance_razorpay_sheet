...
def get_start_date(sheet, sheet_name, col='Start Date'):
    ...
    if find_sheet(sheet, sheet_name):
        ...
        df.sort_values([col], inplace=True, ascending=True)
...
def add_data(worksheet, new_data, columns=None):
    ...
    if new_data :
        ...
        df.fillna(df.select_dtypes(include=['number']).apply(lambda x: 0), inplace=True)     
        df.fillna(df.select_dtypes(exclude=['number']).apply(lambda x: ""), inplace=True)
